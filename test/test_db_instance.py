...
def test_get_optuna_db_failure():
    ...
    with patch("docktuna.optuna_db.db_instance.OptunaDatabase") as MockOptunaDatabase:
        ...
        type(mock_instance).storage = PropertyMock(side_effect=Exception("DB connection failed"))

        # Ensure a fresh start
        get_optuna_db.__globals__["OPTUNA_DB"] = None  

        with pytest.raises(RuntimeError, match="Failed to initialize Optuna database: DB connection failed"):
